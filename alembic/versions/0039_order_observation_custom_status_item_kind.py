...
nenhuma depende de rollback isolado da outra nesta etapa:
  ...
  - `orders.observation*` (4 colunas): observação operacional da
    comanda + auditoria DEDICADA (quem/quando editou por ÚLTIMO — nunca
    reaproveita `orders.updated_at`, que já muda por pagamento/produto/
    status e não serviria como "última edição da observação"). Mesmo
    padrão de snapshot já usado em `order_product_items.product_name`
    (`observation_updated_by_name`). Sem coluna de versão nova — a
    concorrência é resolvida no service layer com um "expected
    timestamp" opcional vindo do cliente (ver `services/orders.py::
    update_observation`), não com uma trava de banco.
  ...
  - `appointment_custom_statuses` (+ `appointments.custom_status_id`):
    ...
    `set_active(False)` só some da lista de seleção.
